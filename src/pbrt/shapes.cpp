...
pstd::vector<Shape> Shape::Create(
    ...
    // Create multiple-_Shape_ types
        ...
        std::optional<ParametricWord> expanded = definition->Expand(loc);
        ...
        LOG_VERBOSE(
            "Parametric L-system expanded to %d modules and generated "
            "%d branch segments / %d renderable Curve shapes.",
            int(expanded->size()),
            int(turtle->branches.size()),
            int(shapes.size()));

        LOG_VERBOSE(
            "Expanded parametric word: %s",
            ParametricWordToString(*expanded).c_str());
    }
    else
