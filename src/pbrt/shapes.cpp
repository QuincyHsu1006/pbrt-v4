...
pstd::vector<Shape> Shape::Create(
    ...
    // Create multiple-_Shape_ types
        ...
        if (definition) {
            ...
            if (expanded) {
                ...
                if (splitDepth < 0 || splitDepth > 10) {
                    Error(loc,
                        "L-system \"splitdepth\" must be in [0, 10]; "
                        "got %d.",
                        splitDepth);
                    return shapes;
                }
                ...
                LOG_VERBOSE(
                    "L-system expanded to %d symbols, generated %d branch skeleton "
                    "segments and %d renderable curve shapes.",
                    int(expanded->size()),
        ...
        // Parser-only version: no geometry yet.
    }
    else
