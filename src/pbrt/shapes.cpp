...
pstd::vector<Shape> Shape::Create(
    ...
    // Create multiple-_Shape_ types
        ...
        std::optional<LSystemDefinition> definition =
            LSystemDefinition::Create(parameters, loc);
        if (!definition) return shapes;

        std::optional<std::string> expanded = definition->Expand(loc);
        if (!expanded) return shapes;

        std::optional<TurtleResult> turtle =
            TurtleInterpreter::Interpret(*expanded, *definition, loc);
        if(!turtle) return shapes;

        std::string geometry = parameters.GetOneString("geometry", "all");

        bool buildBranches = false;
        bool buildLeaves = false;
        if (geometry == "all") {
            buildBranches = true;
            buildLeaves = true;
        }
        else if (geometry == "branches") {
            buildBranches = true;
        }
        else if (geometry == "leaves") {
            buildLeaves = true;
        } else {
            Error(loc,
                "Unsupported L-system \"geometry\" value \"%s\". "
                "Expected \"all\", \"branches\", or \"leaves\".",
                geometry.c_str());
            return shapes;
        }
        ...
        int splitDepth = parameters.GetOneInt("splitdepth", 0);
        if (splitDepth < 0 || splitDepth > 10) {
            Error(loc, "L-system \"splitdepth\" must be in [0, 10]; got %d.", splitDepth);
            return shapes;
        }

        // int leafSplitDepth = parameters.GetOneInt("leafsplitdepth", 1);
        // if (leafSplitDepth < 0 || leafSplitDepth > 10) {
        //     Error(loc, "L-system \"leafsplitdepth\" must be in [0, 10]; got %d.", leafSplitDepth);
        //     return shapes;
        // }

        std::string branchTypeName = parameters.GetOneString( "branchtype", "cylinder");

        CurveType branchType;
        if (branchTypeName == "cylinder")
            branchType = CurveType::Cylinder;
        else if (branchTypeName == "flat")
            branchType = CurveType::Flat;
        else {
            Error(loc,
                "Unsupported L-system branchtype \"%s\"."
                "Only Support \"cylinder\" and \"flat\" for branches.",
                branchTypeName.c_str());
            return shapes;
        }

        // Number of centerline intervals in one leaf. Four intervals produce:
        //   8 vertices and 6 triangles per leaf.
        int leafSegments =
            parameters.GetOneInt("leafsegments", 4);
        if (leafSegments < 2 || leafSegments > 16) {
            Error(loc,
                "L-system \"leafsegments\" must be in [2, 16]; got %d.",
                leafSegments);
            return shapes;
        }

        // Maximum centerline displacement, as a fraction of leaf length.
        Float leafBend =
            parameters.GetOneFloat("leafbend", 0.08f);
        if (!std::isfinite(leafBend) ||
            leafBend < -0.5f || leafBend > 0.5f) {
            Error(loc,
                "L-system \"leafbend\" must be finite and in "
                "[-0.5, 0.5]; got %f.",
                leafBend);
            return shapes;
        }

        int branchShapeCount = 0;
        int leafTriangleCount = 0;

        // Branch geometry
        if(buildBranches){
            pstd::span<const Normal3f> noNormals;

            for (const BranchSegment &branch : turtle->branches) {
                Vector3f delta = branch.end - branch.start;

                if (LengthSquared(delta) == 0) {
                    Warning(loc,
                            "Skipping zero-length L-system branch generated by "
                            "symbol %d.",
                            int(branch.sourceSymbolIndex));
                    continue;
                }

                // Represent the straight BranchSegment as a cubic Bezier curve.
                // Collinear points at 1/3 and 2/3 preserve an exactly straight
                // centerline while satisfying pbrt's cubic Curve interface.
                pstd::array<Point3f, 4> controlPoints = {
                    branch.start,
                    branch.start + delta / 3.f,
                    branch.start + 2.f * delta / 3.f,
                    branch.end
                };

                // pbrt Curve widths are diameters, while BranchSegment stores radii.
                Float width0 = 2.f * branch.radiusStart;
                Float width1 = 2.f * branch.radiusEnd;

                pstd::vector<Shape> branchCurves =
                    CreateCurve(renderFromObject,
                                objectFromRender,
                                reverseOrientation,
                                controlPoints,
                                width0,
                ...
                branchShapeCount += int(branchCurves.size());
        ...
        //Leaf Geometry
        if(buildLeaves && !turtle->leaves.empty()){
            ...
            // Per leaf:
            //   vertices  = base + 2 * (leafSegments - 1) + tip
            //             = 2 * leafSegments
            //   triangles = base cap + 2 * (leafSegments - 2) + tip cap
            //             = 2 * leafSegments - 2
            const int verticesPerLeaf = 2 * leafSegments;
            const int trianglesPerLeaf = 2 * leafSegments - 2;

            positions.reserve(turtle->leaves.size() * verticesPerLeaf);
            normals.reserve(turtle->leaves.size() * verticesPerLeaf);
            uvs.reserve(turtle->leaves.size() * verticesPerLeaf);
            indices.reserve(
                turtle->leaves.size() * trianglesPerLeaf * 3);

            for (const LeafInstance &leaf : turtle->leaves) {
                if (leaf.length <= 0 || leaf.width <= 0 ||
                    LengthSquared(leaf.heading) == 0 ||
                    LengthSquared(leaf.left) == 0 ||
                    LengthSquared(leaf.up) == 0) {
                    Warning(loc,
                            "Skipping invalid L-system leaf generated by "
                            "symbol %d.",
                            int(leaf.sourceSymbolIndex));
                    continue;
                }

                Vector3f H = Normalize(leaf.heading);
                Vector3f S = Normalize(leaf.left);
                Vector3f U = Normalize(leaf.up);

                int baseVertex = int(positions.size());

                auto centerAt = [&](Float t) {
                    // sin(pi*t) is zero at the base/tip and reaches its maximum
                    // near the center of the leaf.
                    Float bendOffset =
                        leafBend * leaf.length * std::sin(Pi * t);
                ...
                auto normalAt = [&](Float t) {
                    // Derivative of the bent centerline:
                    // dP/dt = length * H
                    //       + bend * length * pi * cos(pi*t) * U
                    Vector3f tangent =
                        H + leafBend * Pi * std::cos(Pi * t) * U;

                    Vector3f n = Cross(tangent, S);
                    if (LengthSquared(n) == 0)
                        n = U;

                    return Normal3f(Normalize(n));
                };

                // Base vertex.
                positions.push_back(centerAt(0.f));
                normals.push_back(normalAt(0.f));
                uvs.push_back(Point2f(0.5f, 0.f));

                // Interior left/right pairs.
                for (int i = 1; i < leafSegments; ++i) {
                    Float t = Float(i) / Float(leafSegments);

                    // Smooth pointed-leaf width profile. The stored leaf.width is
                    // the full maximum width, so this value is a half width.
                    Float profile =
                        std::pow(std::sin(Pi * t), 0.75f);
                    Float halfWidth =
                        0.5f * leaf.width * profile;

                    Point3f center = centerAt(t);
                    Normal3f normal = normalAt(t);

                    positions.push_back(center + halfWidth * S);
                    normals.push_back(normal);
                    uvs.push_back(Point2f(0.f, t));

                    positions.push_back(center - halfWidth * S);
                    normals.push_back(normal);
                    uvs.push_back(Point2f(1.f, t));
                }

                // Tip vertex.
                positions.push_back(centerAt(1.f));
                normals.push_back(normalAt(1.f));
                uvs.push_back(Point2f(0.5f, 1.f));

                int tipVertex = baseVertex + verticesPerLeaf - 1;

                // Base triangle. Winding points approximately along +leaf.up.
                int firstLeft = baseVertex + 1;
                int firstRight = baseVertex + 2;

                indices.push_back(baseVertex);
                indices.push_back(firstRight);
                indices.push_back(firstLeft);

                // Two triangles between each adjacent pair of cross sections.
                for (int section = 1;
                    section < leafSegments - 1;
                    ++section) {
                    int left0 =
                        baseVertex + 1 + 2 * (section - 1);
                    int right0 = left0 + 1;
                    int left1 = left0 + 2;
                    int right1 = left1 + 1;

                    indices.push_back(left0);
                    indices.push_back(right0);
                    indices.push_back(right1);

                    indices.push_back(left0);
                    indices.push_back(right1);
                    indices.push_back(left1);
                }

                // Tip triangle.
                int lastLeft =
                    baseVertex + 1 + 2 * (leafSegments - 2);
                int lastRight = lastLeft + 1;

                indices.push_back(lastLeft);
                indices.push_back(lastRight);
                indices.push_back(tipVertex);
            }

            if (!indices.empty()) {
                leafTriangleCount = int(indices.size() / 3);

                // Tangents and face indices are optional here.
                std::vector<Vector3f> tangents;
                std::vector<int> faceIndices;

                TriangleMesh *leafMesh =
                    alloc.new_object<TriangleMesh>(
                        *renderFromObject,
                        reverseOrientation,
                        std::move(indices),
                        std::move(positions),
                ...
                pstd::vector<Shape> leafTriangles =
                    Triangle::CreateTriangles(leafMesh, alloc);
        ...
        LOG_VERBOSE("Parsed L-system: %s", definition->ToString().c_str());
        LOG_VERBOSE(
            "L-system geometry=\"%s\": expanded to %d symbols, "
            "generated %d branch Curve shapes and %d leaf triangles "
            "(%d LeafInstance records).",
            geometry.c_str(),
            int(expanded->size()),
            branchShapeCount,
            leafTriangleCount,
            int(turtle->leaves.size()));
